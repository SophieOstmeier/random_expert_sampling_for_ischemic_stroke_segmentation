...
import SimpleITK
from nnunet.training.dataloading.dataset_loading_multirater_sampling import DataLoader3D_random, unpack_dataset, load_dataset_multi_rater
...
class nnUNetTrainerV2_random_data_loader_3rater_x(nnUNetTrainerV2):
        ...
        if not self.was_initialized:
            ...
            if training:
                ...
                if self.unpack_data:
                    ...
                    for i in glob.glob(self.dataset_directory + "/" + self.plans[
                        'data_identifier'] + "_stage%d_rater*" % self.stage):
                        unpack_dataset(i)
                    print("done")
                ### for multirater training
                else:
            ...
            torch.cuda.set_device(
                self.gpu_id_to_use)  # added from brian to have tensor and network both on gpu_id_to_use
